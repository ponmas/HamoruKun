...
public static class SoundLibrary
{
    ...
    public static float AnalyzeSound(AudioSource ac, int qSamples, float threshold)
    {
       ...
       // Debug.Log("max"+maxV);
        ...
        float pitchValue = freqN * (AudioSettings.outputSampleRate / 2.0f) / qSamples;
        return pitchValue;
    }
    ...
    public static string ConvertScaleToString(float scale)
    {
        ...
        int s = (int)(scale * precision);
        if (scale * precision - s >= 0.5) s += 1; // 四捨五入


        int smod = s % (12 * precision); // 音階
        int soct = (s + 33 * precision) / (12 * precision); // オクターブ
    ...
    public static void ScaleWave(float[] scale, int size, LineRenderer line)
    {
        ...
        {
            line.SetPosition(i, new Vector3(x, -80.0f + scale[i] * 4.0f, 0));
            x += 1.0f;
        }
    }
}